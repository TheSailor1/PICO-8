...
function deckcheck()
		for i=1,#shfdeck do
		? "\#0"..shfdeck[i].rank.." "..shfdeck[i].suit,100,2+(i-1)*8
	end
	for i=1,#shfdeck2 do
		? "\#0\f9"..shfdeck2[i].rank.." "..shfdeck2[i].suit,80,2+(i-1)*8
	end
	for i=1,#deck do
		? "\#0\f2"..deck[i].rank.." "..deck[i].suit,60,2+(i-1)*8
	end
end
...
function drw_deck(_x,_y)
	drw_bcard(_x,_y)
	? "__",_x+36,(_y+20)-(room*2),roomcol[room]
	? #shfdeck,_x+36,(_y+28)-(room*2),roomcol[room]
	? "__",_x+36,(_y+31)-(room*2),roomcol[room]
--	? "room:"..room,_x+8,_y+26,7
end

function drw_hand()
	if pullcard==false then
		for i=1,#hand do
			drw_fcard(
				handpos[i].x,
				handpos[i].y,
...
function upd_cursor()
	...
	if #hand>0 then
		--right
		if btnp(1) then
			if cursel==4 and not hasrun then
				cursel=5
			elseif cursel==2 and 
										#hand==4 and
										not hasrun then
				cursel=5
			elseif cursel<#handpos and 
						cursel<#hand then
				cursel+=1
			else
				cursel=1
		...
		--up
		if btnp(2) then
			if cursel==3 and #hand>=3 then
				cursel=1
			elseif cursel==4 and #hand==4 then
				cursel=2
			end
		end
		...
		--down
		if btnp(3) then
			if cursel==1 and #hand>=3 then
				cursel=3
			elseif cursel==2 and #hand==4 then
				cursel=4
			end
		end
	end
...
function set_deck()
	for i=#deck,1,-1 do
		if deck[i].suit=="hrt" or
					deck[i].suit=="dmd" then
			if deck[i].rank>=11 then
				deli(deck,i)
			end
		end
	end
...
function tak_cards(_v)
	for i=1,_v do
		shfdeck[1].x=handpos[1].x
		shfdeck[1].y=handpos[1].y
		add(hand,shfdeck[1])
		deli(shfdeck,1)
	end
	pullcard=false
...
function mk_discard()
	discard.rank=hand[cursel].rank
	discard.suit=hand[cursel].suit
	discard.x=handpos[cursel].x
	discard.y=handpos[cursel].y
	async(discards)
end
